...
class AgentSettings(BaseModel):
    ...
    journal_chars: int = Field(
        ...
        le=500_000,
        description=(
            "Сколько знаков журнала находок кладётся в постановку каждого шага. "
            "Журнал растёт весь прогон; что не помещается, заменяется "
            "оглавлением, и агент дочитывает нужную запись из файла."
        ),
    )
    ...
    steps_agent: int = Field(
        default=40,
        ge=5,
        le=200,
        description=(
            "Сколько действий даётся агенту в режиме «Агент» — на всю задачу "
            "целиком (найти источники, прочитать, посчитать, свести отчёт). "
            "Одно действие — один вызов инструмента. Мало (20) — глубокое "
            "исследование упирается в предел раньше, чем собран вывод; много — "
            "дороже и дольше, если задача простая."
        ),
    )

    steps_plan: int = Field(
        default=20,
        ge=5,
        le=100,
        description=(
            "Сколько действий даётся на КАЖДЫЙ пункт плана в режиме «План». Это "
            "не бюджет на всю задачу: пунктов может быть много, и общий счёт — "
            "это сумма по пунктам. Держите меньше, чем у «Агента»: один пункт — "
            "это подзадача, а не вся работа."
        ),
    )
...
class MCPServerConfig(BaseModel):
    ...
    env: Dict[str, str] = Field(
        ...
        description="Environment variables for stdio command, e.g. API tokens",
    )
